...
5. **GIGACHAT_DISABLE_SSL_CHECK** (только при проблемах с SSL сертификатами)
   - Значение: `true` для отключения проверки SSL сертификатов
   - ⚠️ Используйте только если GigaChat API возвращает ошибки SSL
   - По умолчанию: не установлено (проверка SSL включена)
